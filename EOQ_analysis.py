...
# updated to 
last_four_weeks_data = merged_data[(merged_data['Date'] >= four_weeks_ago) & (merged_data['Date'] < current_date)]
...
for org_name in organizations:
    ...
    for product_name in products:
        ...
        if not product_data.empty:
            ...
            graph_filename = f'./graphs/{org_name}_{product_name}_graph.png'
            plt.savefig(graph_filename, format='png')
            plt.close()

            # Insert the graph into the "Graphs" sheet
            graph_sheet.insert_image(start_row, 0, graph_filename, {'x_offset': 25, 'y_offset': 25})

            start_row += 30  # Adjust the starting row for the next graph
for title, data in print_statements.items():
    if isinstance(data, pd.DataFrame):
        # Generate the graph
        plt.figure()
        ...
        graph_sheet.insert_image(start_row, 0, f'./graphs/{title}_graph.png', {'x_offset': 25, 'y_offset': 25})
        plt.savefig(f'./graphs/{title}_graph.png', format='png')
        plt.close()
