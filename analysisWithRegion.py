...
weekly_sales = sales_data.groupby(['Region', 'Product Name', 'Week', 'Date'])['Quantity'].sum().reset_index().sort_values(by=['Date'], ascending=True)

# Step 4: Find the Demanded Products for the Last Two Weeks in each region
print(weekly_sales['Week'].unique())
last_two_weeks = weekly_sales['Week'].unique()[-2:]
last_two_weeks_dates = weekly_sales[weekly_sales['Week'].isin(last_two_weeks)]['Date'].unique()
start_date = last_two_weeks_dates.min().date()
end_date = last_two_weeks_dates.max().date()

print(f"Last Two Weeks of Analysis: {start_date} to {end_date}")
